...
export function parsePokepaste(text: string): ParsedMon[] {
  ...
  return blocks.map(block => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      else if (line.startsWith('Level:')) level = line.replace('Level:', '').trim();
      else if (line.startsWith('EVs:')) evs = line.replace('EVs:', '').trim();
      else if (line.endsWith('Nature')) nature = line.replace('Nature', '').trim();
      else if (line.startsWith('-')) moves.push(line.replace('-', '').trim());
    }

    return { species, item, ability, level, nature, evs, moves } as ParsedMon;
  }).filter(Boolean) as ParsedMon[];
}
