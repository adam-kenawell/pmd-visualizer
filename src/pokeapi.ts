...
const pokeDataCache: Record<string, any> = {};
const moveDataCache: Record<string, string> = {};
...
export async function fetchPokemonData(species: string): Promise<any> {
  let slug = species.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  const lookup = FORM_MAP[slug] || slug;
  if (pokeDataCache[lookup]) return pokeDataCache[lookup];
  try {
...
export async function nameToDexId(species: string): Promise<number> {
  const data = await fetchPokemonData(species);
  return data?.id || 0;
}

export async function fetchMoveType(moveName: string): Promise<string> {
  const slug = moveName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  if (moveDataCache[slug]) return moveDataCache[slug];
  try {
